...
type
  TfrmVidDostavkiSpr = class(TForm)
  ...
  private
    ...
    frmSkladEdit : TfrmVidDostavkiSprEdit;
    procedure SkladEdit;
...
var
  frmVidDostavkiSpr: TfrmVidDostavkiSpr;

implementation

{$R *.dfm}

procedure TfrmVidDostavkiSpr.btnDeleteClick(Sender: TObject);
begin
try
...
procedure TfrmVidDostavkiSpr.btnInsertClick(Sender: TObject);
begin
try
  cdsSklad.Insert;
  SkladEdit;
...
procedure TfrmVidDostavkiSpr.SkladEdit;
begin
try
...
procedure TfrmVidDostavkiSpr.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmVidDostavkiSpr.FormCreate(Sender: TObject);
begin
  cdsSklad.Open;
  frmSkladEdit := TfrmVidDostavkiSprEdit.Create (self);
  frmSkladEdit.dsSkladEdit.Dataset := cdsSklad;
end;

procedure TfrmVidDostavkiSpr.UnfDBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
    vk_return :begin
...
procedure TfrmVidDostavkiSpr.btnEditClick(Sender: TObject);
begin
try
  cdsSklad.Edit;
  SkladEdit;
...
procedure TfrmVidDostavkiSpr.cdsSkladReconcileError(
...
procedure TfrmVidDostavkiSpr.actRefreshExecute(Sender: TObject);
begin
try
  cdsSklad.Refresh;
except
...
procedure TfrmVidDostavkiSpr.cdsSkladAfterInsert(DataSet: TDataSet);
begin
try
  cdsSklad.FieldByName('oid').asInteger :=
    dmdEx.GetGenValue('oid_gen');
  cdsSklad.FieldByName('delmarked').asInteger := 0;
  cdsSklad.FieldByName('isdefault').asInteger := 0;
except
  AssertInternal('A6A8E837-8C52-40A8-9984-F0EF4218BE6C');
end;
end;

procedure TfrmVidDostavkiSpr.cdsSkladBeforePost(DataSet: TDataSet);
begin
...
procedure TfrmVidDostavkiSpr.dbgClientDblClick(Sender: TObject);
begin
  SkladEdit;
end;

procedure TfrmVidDostavkiSpr.dbgClientKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
    vk_return :begin
...
function TfrmVidDostavkiSpr.ShowModalForInsert: integer;
begin
...
procedure TfrmVidDostavkiSpr.btnOkClick(Sender: TObject);
begin
  close;
end;

...
type
  ...
  private
    ...
    FOidNakl : integer ; //номер типа "Расходная накладная"
...
try
  ...
  try
    ...
    on EVariantTypeCastError do begin
      AssertInternal('A8BBF66C-32ED-4F77-B66A-9DA5B6C23200'+ ' не указан склад по умолчанию');
    end;
  end;
...
begin
  { DONE : Переделать спецификации }
	//показать форму спецификации
...
begin
  try
    //instead of date_from
    qeNaklo.SetSQL('where', 'id_nakl=' + inttostr(id_nakl), 3); //только наш документ
...
try
  ...
  if not ProcessOnlySecurityMeasures then begin
    self.Caption := UpdateCaptionVladClientManager('Расходные накладные:',setT);
  end;
except
...
begin
  ...
  DataSet.FieldByName('fr').asString := 'б/н';
...
begin
	...
	if DataSet.FieldByName('vid_dostavki').IsNull then begin
		showmessage ('Заполните поле ''Вид доставки''');
		abort;
	end;
end;
...
try
  ...
  OpenDocument(self,
    ...
    dmdEx.GetOidObjects('ВОЗВРАТ',-100),
    0); 
except
...
var
  iXozOper : integer;
begin
  iXozOper:=dmdEx.GetParamCached('ХОЗЯЙСТВЕННЫЕ ОПЕРАЦИИ',
    'select oid from GET_OID_OBJECTS_PC('+quotedstr('ХОЗЯЙСТВЕННЫЕ ОПЕРАЦИИ')+',-100)',
    'oid');
  Result :=dmdEx.GetParamCached('РАСХОДНАЯ НАКЛАДНАЯ НА ТОВАР',
    'select oid from GET_OID_OBJECTS_PC('
      +quotedstr('РАСХОДНАЯ НАКЛАДНАЯ НА ТОВАР')
      +','+inttostr(iXozOper)+')',
    'oid');
end;

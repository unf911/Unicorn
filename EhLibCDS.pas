    ...
    if TCustomDBGridEh(Sender).SortLocal then
      ...
      begin
        ...
        if (DataSet is TClientDataSet) then begin
          if TClientDataSet(DataSet).MasterFields<>'' then begin
            IndexFields := TClientDataSet(DataSet).MasterFields+ ';';
          end;
        end;
...
{$ENDIF}
        begin
          ...
          if CDS.IndexDefs.IndexOf('SortIndexEh') >= 0 then begin
            if (DataSet is TClientDataSet) then begin
              if TClientDataSet(DataSet).MasterFields<>'' then begin
                CDS.AddIndex('SortKeyEh', TClientDataSet(DataSet).MasterFields, [], '');
                CDS.IndexName := 'SortKeyEh';
              end;
            end;

...
var
	frmVozvratPost: TfrmVozvratPost;
...
procedure TfrmVozvratPost.FormClose(Sender: TObject; var Action: TCloseAction);
begin
	Action := caFree;
end;

procedure TfrmVozvratPost.actSettingsExecute(Sender: TObject);
begin
  actSettings.checked := setT.ProcessSettingsChange2(DefaultOpen);
end;

procedure TfrmVozvratPost.actGrid1ListShowExecute(Sender: TObject);
begin
...
procedure TfrmVozvratPost.dbgNaklrDblClick(Sender: TObject);
begin
  actGrid1ListShow.Execute;
end;

procedure TfrmVozvratPost.DefaultStartup;
begin
...
function  TfrmVozvratPost.DefaultOpen:boolean;
begin
  ProcessSettings;
...
procedure TfrmVozvratPost.FormCreate(Sender: TObject);
begin
try
  dmdEx.StartWaiting;
  ...
  dmdEx.StopWaiting;
  frmEdit := TfrmVozvratPostEdit.Create(self);
  frmEdit.dsNakloEdit.DataSet := cdsNaklo;

  frmDet := TfrmVozvratPostDet.Create(self);
...
procedure TfrmVozvratPost.qurNaklrCommit;
begin
  PostAndApplyOrCancel(cdsNaklo);
end;

procedure TfrmVozvratPost.actRefreshExecute(Sender: TObject);
begin
try
  dmdEx.StartWaiting;
	qurNaklrCommit;
...
procedure TfrmVozvratPost.FormDestroy(Sender: TObject);
begin
try
...
procedure TfrmVozvratPost.actGrid2ListShowExecute(Sender: TObject);
begin
...
procedure TfrmVozvratPost.FormDeactivate(Sender: TObject);
begin
	qurNaklrCommit;
end;

procedure TfrmVozvratPost.cdsNakloAfterInsert(DataSet: TDataSet);
begin
  dmdEx.StartWaiting;
...
procedure TfrmVozvratPost.cdsNaklotAfterInsert(DataSet: TDataSet);
var
  intGen : integer;
begin
  if cdsNaklo.FieldByName('id_nakl').AsInteger=0 then begin
...
procedure TfrmVozvratPost.cdsNaklotBeforeInsert(DataSet: TDataSet);
begin
...
procedure TfrmVozvratPost.cdsNaklotBeforeScroll(DataSet: TDataSet);
begin
try
...
procedure TfrmVozvratPost.cdsNakloBeforeScroll(DataSet: TDataSet);
begin
try
  ApplyOrCancel (Dataset);
except
...
procedure TfrmVozvratPost.dbgNaklotDblClick(Sender: TObject);
begin
  actGrid2ListShow.Execute;
end;

procedure TfrmVozvratPost.dbgNakloDblClick(Sender: TObject);
begin
  actGrid1ListShow.Execute;
end;


procedure TfrmVozvratPost.dspNakloBeforeUpdateRecord(Sender: TObject;
  SourceDS: TDataSet; DeltaDS: TCustomClientDataSet;
  UpdateKind: TUpdateKind; var Applied: Boolean);
begin
...
procedure TfrmVozvratPost.actShowDeletedExecute(Sender: TObject);
begin
  dmdEx.StartWaiting;
...
procedure TfrmVozvratPost.ProcessShowDeleted;
begin
  if actShowDeleted.Checked then begin
...
procedure TfrmVozvratPost.ShowDetail2;
begin
...
procedure TfrmVozvratPost.cdsNakloReconcileError(DataSet: TCustomClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TfrmVozvratPost.ProcessSettings(ProcessOnlySecurityMeasures:boolean=false);
begin
try
...
procedure TfrmVozvratPost.cdsNaklotReconcileError(DataSet: TCustomClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
  showmessage (e.Message);
...
procedure TfrmVozvratPost.actBlockExecute(Sender: TObject);
begin
...
function TfrmVozvratPost.ShowDetail1:integer;
begin
  Result := dmdEx.ShowDetail1(dsNaklo.DataSet,frmEdit,actRecountNakl);
end;

procedure TfrmVozvratPost.actUndeleteNaklExecute(Sender: TObject);
begin
  if actShowDeleted.Checked then begin
...
procedure TfrmVozvratPost.dbgNakloKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
try
  case key of
...
procedure TfrmVozvratPost.dbgNaklotKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
		vk_return : begin
...
procedure TfrmVozvratPost.dspNakloGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: String);
begin
  TableName := 'VOZVRAT_POST_VW';
end;

procedure TfrmVozvratPost.actAddDetExecute(Sender: TObject);
var
...
procedure TfrmVozvratPost.cdsNaklotCENANDSChange(Sender: TField);
begin
...
procedure TfrmVozvratPost.cdsNaklotKOLOTPChange(Sender: TField);
begin
  if not Semaphore then begin
    Semaphore := true;
...
procedure TfrmVozvratPost.cdsNaklotSUMAChange(Sender: TField);
begin
  if not Semaphore then begin
    Semaphore := true;
...
procedure TfrmVozvratPost.cdsNaklotSUMANDSChange(Sender: TField);
begin
  if not Semaphore then begin
    Semaphore := true;
...
procedure TfrmVozvratPost.cdsNaklotCENAChange(Sender: TField);
begin
  if not Semaphore then begin
    Semaphore := true;
...
procedure TfrmVozvratPost.cdsNaklotCalcFields(DataSet: TDataSet);
begin
...
procedure TfrmVozvratPost.cdsNaklotAfterDelete(DataSet: TDataSet);
begin
  actRecountNakl.Execute;
end;

procedure TfrmVozvratPost.dspNaklotGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: String);
begin
  TableName := 'VOZVRAT_POST_DET_VW';
end;


procedure TfrmVozvratPost.FillSettings;
begin
...
procedure TfrmVozvratPost.actPostExecute(Sender: TObject);
begin
  dmdEx.PostNakl(
...
procedure TfrmVozvratPost.actShowParentNaklpExecute(Sender: TObject);
begin
  ...
  OpenDocument(self,
    ...
    dmdEx.GetOidObjects('ПРИХОДНЫЕ НАКЛАДНЫЕ',-100),
    0);
end;


procedure TfrmVozvratPost.actUnpostExecute(Sender: TObject);
begin
  dmdEx.UnPostNakl(
    TIntegerField(cdsNaklo.FieldByName('posted')),
    sdsUnPost,
...
procedure TfrmVozvratPost.actShowNakloExecute(Sender: TObject);
var
  iTip : integer;
begin
  if dsNaklo.DataSet.Eof and dsNaklo.DataSet.Bof then begin
...
procedure TfrmVozvratPost.actShowRepNalDolgExecute(Sender: TObject);
var
...
try
  ...
  AssertInternal ('A23CDB9F-55DB-4E4D-9F23-B4F11538573B');
end;
end;

procedure TfrmVozvratPost.actRecountNaklExecute(Sender: TObject);
begin

...
begin
  ...
  if ((tip<>1) and (tip<>2) and (tip<>-110 /*Возврат*/)) then begin
  ...
  if (:tip in(1,2, -110 /*Возврат*/)) then begin
  ...
  if (parent_id_nakl is not null ) then begin
    ...
    if (parent_tip=:tip_vozvrat) then begin
      ...
      if (:id_sklad_otgr<>:id_sklad) then begin
        execute procedure error ('Склад не совпадает со складом отгрузки, на которую делалася возврат. ' || trim(sklad_name_otgr));
      end
    end
  ...
  select oid
    from GET_OID_OBJECTS_PC('УБЫТОК ОТ ВОЗВРАТОВ ТОВАРА КЛИЕНТАМИ',:id_statya_dr_tip)
    into :id_statya_rasx_ot_vozvratov;
  select oid
    from GET_OID_OBJECTS_PC('ДОХОД ЗА ДОСТАВКУ ТОВАРА',:id_statya_dr_tip)
  ...
  if (tip=1) then begin /**/
    if ((id_schet is null) or (schet_name='ФИНАНСОВЫЙ РЕЗУЛЬТАТ') ) then begin
      ...
      /*Убыток от возвратов = сумма накладной - сумма себестоимости*/
      if (parent_tip=:tip_vozvrat) then begin
        insert into provodki (id_schet,dat,tip,
            kredit,id_nakl,
            sub1,sub2,sub3,sub4,sub5,sub6,attr1)
...
GRANT SELECT ON VOZVRAT_VW TO PROCEDURE NAKLP_POST_PC;

GRANT SELECT ON NAKLOT TO PROCEDURE NAKLP_POST_PC;

GRANT SELECT ON OBJECTS TO PROCEDURE NAKLP_POST_PC;
...
GRANT EXECUTE ON PROCEDURE GET_SUM_TOVARWITHNDS TO PROCEDURE NAKLP_POST_PC;

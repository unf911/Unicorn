...
begin
  ...
  for
    select pr.sub1, pr.sub2,pr.sub3,/*$$IBEC$$ pr.sub4,pr.sub5, $$IBEC$$*/
      ...
      into :sub1,:sub2,:id_partiya,/*$$IBEC$$ :sub4,:sub5, $$IBEC$$*/:kredit_kolotp,:kredit
  do begin
    if ((z(sum_partiya_kolotp)+z(kredit_kolotp))<=kolotp) then begin
      partiya_kolotp_vozvrat=-1*:kredit_kolotp;
      sebest_partiya=-1*:kredit;
    end else begin
      partiya_kolotp_vozvrat=-1*Round(:kolotp-:sum_partiya_kolotp,3);
      sebest_partiya=Round(dzero(:kredit,:kredit_kolotp,0)*:partiya_kolotp_vozvrat,2);
    end
    sum_partiya_kolotp=Round(z(sum_partiya_kolotp)+abs(z(partiya_kolotp_vozvrat)),3);
    insert into provodki(id_schet,id_nakl,tip,dat,
        kredit,kredit_kolotp,
        sub1,sub2,sub3/*$$IBEC$$ ,sub4,sub5 $$IBEC$$*/,attr1)
      values(:id_schet_sebest,:id_nakl, :tip,:dat,
        :sebest_partiya,:partiya_kolotp_vozvrat,
        :sub1,:sub2, :id_partiya/*$$IBEC$$ ,:sub4,:sub5 $$IBEC$$*/,:id_nakld);
    if (sum_partiya_kolotp>=kolotp) then begin
      leave;
    end
  end
  if (sum_partiya_kolotp<:kolotp) then begin
    sError ='Ошибка в n_s2_v_s_c. Нет себестоимости для id_tovar=' || coalesce(:id_tovar,0) || '. id_nakl=' ||coalesce(:id_nakl,0);
    execute procedure error(sError);
  end
...
GRANT SELECT ON NAKLOT TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT SELECT ON NAKLO TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT EXECUTE ON PROCEDURE GET_OID_OBJECTS_PC TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT EXECUTE ON PROCEDURE ERROR TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT EXECUTE ON PROCEDURE SKLAD_SEBEST_SETTINGS_PC TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT SELECT ON SPR_UCHET_POLITICS_VW TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT SELECT ON VOZVRAT_VW TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT SELECT,INSERT ON PROVODKI TO PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC;

GRANT EXECUTE ON PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC TO PROCEDURE NAKLP_POST_PC;
GRANT EXECUTE ON PROCEDURE NAKLOT_SEBEST2_VOZVR_SKL_PC TO SYSDBA;

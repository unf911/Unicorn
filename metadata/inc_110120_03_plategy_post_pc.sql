...
begin
  ...
  select oid
    ...
    into :id_statya_ras;
  select oid
    from GET_OID_OBJECTS_PC('ТРАНШ В ДОЛГАХ ЗА ФИНУСЛУГИ ВОЗВРАТ',:id_xoz_oper_tip)
    into :tip_transh_vovz_pok_usl;
  select oid
    from GET_OID_OBJECTS_PC('ТРАНШ В ДОЛГАХ ЗА УСЛУГИ ВОЗВРАТ',:id_xoz_oper_tip)
...
/*Отрицательные платежи, т.е. возвраты*/
  ...
  if (((id_schet is null) or (schet_name='ДОЛГИ ЗА АРЕНДУ'))
    and (xoz_oper_name='ВОЗВРАТ ПОКУПАТЕЛЮ АРЕНДЫ'))
  then begin
  ...
  if (((id_schet is null) or (schet_name='ДОЛГИ ЗА ФИНУСЛУГИ'))

...
CREATE PROCEDURE NAKLOT_SEBEST2_COUNT_PC(
    ID_NAKLD INTEGER,
  ...
  IF (mode=11) THEN BEGIN
    ...
    FOR
      SELECT   pr.kredit_kolotp,
        Round(dzero(pr.kredit,pr.kredit_kolotp,0),4),
        pr.kredit_kolotp,
        pr.sub5,
        pr.kredit
      FROM provodki pr
...
/*$$IBEC$$         IF (id_partiya IS NULL)  THEN BEGIN
  ...
  IF (mode=4) THEN BEGIN
    ...
    SELECT
        ...
        Round(dzero(SUM(pr.kredit),SUM(pr.kredit_kolotp),0),4),
        MIN(pr.sub5)
      FROM provodki pr
      WHERE 
        pr.tip<>1 AND
    ...
    DO BEGIN
      ...
      IF (flag_end_of_partiya=0) THEN BEGIN
       /*Если же середина партии, то брать цену первого прихода*/
        ...
        sebest=Round(Round(dzero(kredit,kredit_kolotp,0),4)*partiya_kolotp,4);

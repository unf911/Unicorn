...
CREATE OR ALTER PROCEDURE VOZVRAT_POST_PC(
    ID_NAKL INTEGER,
    ID_SCHET INTEGER)
AS
...
begin
  ...
  select
    ...
    into
      ...
      :tip, :id_currency ;
  select oid
    from GET_OID_OBJECTS_PC('ВОЗВРАТ',-100)
    into :tip_to_process;
  select oid
  ...
  if ((:tip<>:tip_to_process) or (:tip is null) ) then begin
    execute procedure error ('Этой процедурой можно провести только ''ВОЗВРАТ''');
  end    
  if (id_schet is not null) then begin
    ...
    do begin
      ...
      if (kolotp_parent<kolotp) then begin
        ...
        sError = 'Товара ' || trim(:tovar_name) || ' больше чем в родительской расходной';
        execute procedure error(sError);
      end
    end  /*for*/
    /*Упорядочивание позиций*/
    execute procedure NAKLO_ORDER_PC(:id_nakl);
    /*цена равна продажной цене из родительской расходной*/
    update naklot ot
      set ot.cena=(
...
GRANT EXECUTE ON PROCEDURE NAKLO_AUTOFILL_CN_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT SELECT,UPDATE ON NAKLO TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE GET_OID_OBJECTS_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE SKLAD_SEBEST_SETTINGS_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE ERROR TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE GET_NAME_OBJECTS_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT SELECT,INSERT,DELETE ON PROVODKI TO PROCEDURE VOZVRAT_POST_PC;

GRANT SELECT,UPDATE ON NAKLOT TO PROCEDURE VOZVRAT_POST_PC;

GRANT SELECT ON OBJECTS TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLO_ORDER_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLO_SUM_RECOUNT TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKL_AUTO_OPLAT_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT SELECT ON TOVAR TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLOT_SEBEST2_VOZVRAT_PC TO PROCEDURE VOZVRAT_POST_PC;

GRANT EXECUTE ON PROCEDURE VOZVRAT_POST_PC TO SYSDBA;
GRANT EXECUTE ON PROCEDURE VOZVRAT_POST_PC TO BUXG;
GRANT EXECUTE ON PROCEDURE VOZVRAT_POST_PC TO POWER_USER;

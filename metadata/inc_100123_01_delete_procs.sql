...
RETURNS (
    ...
    F9_NDS NUMERIC(15,4),
    F11_BAZANDSEXPORT NUMERIC(15,4),
    TIP INTEGER)
AS
  ...
  for
   ...
   do begin
     ...
     if  (tip_name='КОРРЕКТИРОВКА') then begin
       f4_viddoc='ПНУ';/*Уточнена Податкова накладна*/
     end
     if (tip_name='НАЛОГОВАЯ ЭКСПОРТНАЯ') then begin
       f4_viddoc='ВДМ';/*Вантажна митна декларация*/
     end
     ...
     F9_NDS = null;
     f11_BAZANDSEXPORT = null;

     /*обычная налоговая*/
     if ((klient_tip is null) and (tip=3002 or tip=0)) then begin
       f6_ipn=ipn;
       F7_SUMWITHNDS=nds;
       F8_BAZANDS=vsego;
     ...
     if (klient_tip='Экспортёр') then begin
       F7_SUMWITHNDS=nds;
       F8_BAZANDS=nds;
       f11_BAZANDSEXPORT=nds;
     end
     /*Услуги от нерезидента*/
     if ((klient_tip='Неплательщик НДС') or (tip=-35)) then begin
       f6_ipn='X';
       F7_SUMWITHNDS=nds;
       F8_BAZANDS=vsego;
       F9_NDS=Round(vsego*nalog_nds,2);       

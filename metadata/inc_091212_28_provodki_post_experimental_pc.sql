  ...
  begin
    ...
    delete from provodki pr
      where pr.id_schet=:id_schet and
        ...
        pr.tip=1 and
        Round(z(pr.debet),6)=0 and
        Round(z(pr.kredit),6)=0 and
        Round(z(pr.debet_kolotp),6)=0 and
        Round(z(pr.kredit_kolotp),6)=0;
    */
...
DESCRIBE PROCEDURE PROVODKI_POST_PC
'
action_id=1 Вставка
action_id=3 Удаление
action_id=4 Пересчёт остатков - пока не используется.
Думал использовать для пересчёта себестоимости ПОСЛЕ
вставки всех проводок';

GRANT EXECUTE ON PROCEDURE GET_NAME_OBJECTS_PC TO PROCEDURE PROVODKI_POST_PC;

GRANT SELECT,INSERT,DELETE,UPDATE ON PROVODKI TO PROCEDURE PROVODKI_POST_PC;

GRANT SELECT ON ATTRIBUTES TO PROCEDURE PROVODKI_POST_PC;

GRANT EXECUTE ON PROCEDURE PROVODKI_POST_PC TO TRIGGER PROVODKI_AD10;
GRANT EXECUTE ON PROCEDURE PROVODKI_POST_PC TO TRIGGER PROVODKI_AI10;
GRANT EXECUTE ON PROCEDURE PROVODKI_POST_PC TO PROCEDURE PROVODKI_OSTATKI_FORCE_PC;
GRANT EXECUTE ON PROCEDURE PROVODKI_POST_PC TO PROCEDURE PROVODKI_OSTATKI_PC;
GRANT EXECUTE ON PROCEDURE PROVODKI_POST_PC TO PROCEDURE PROVODKI_REFRESH_PC;
GRANT EXECUTE ON PROCEDURE PROVODKI_POST_PC TO SYSDBA;

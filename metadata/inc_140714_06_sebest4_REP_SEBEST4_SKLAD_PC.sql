...
CREATE PROCEDURE REP_SEBEST4_SKLAD_PC(
    MODE INTEGER,
    ID_SKLAD_IN INTEGER,
    ID_TOVAR_IN INTEGER,
    ...
    do begin
      ...
      do begin
        for
          ...
          where round(r.kolotp_after,4)<>0  or Round(r.debetafter-r.kreditafter,4)<>0
          group  by sub2
          having round(sum(r.kolotp_after),4)<>0   or
            Round(sum(r.debetafter)-sum(r.kreditafter),4)<>0

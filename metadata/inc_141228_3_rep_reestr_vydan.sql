...
CREATE PROCEDURE REP_REESTR_VYDAN_NNAKL_PC(
    ID_IZG INTEGER,
    DATE_FROM TIMESTAMP,
    DATE_TO TIMESTAMP)
RETURNS (
  ...
  for
   ...
   do begin
     ...
     if (lgoty=1)  then begin
       f6_ipn='400000000000';   /*В реестре выданных накладных в гр.6 должен стоять "Х" для экспорта.*/
       F7_SUMWITHNDS = nds;
       f12_BAZANDSEXPORT = nds;
       f4_viddoc='МДП';/*Вместо ПН07*/
     end
     if (tip_name='НАЛОГОВАЯ ЭКСПОРТНАЯ') then begin
       f6_ipn='300000000000';   /*В реестре выданных накладных в гр.6 должен стоять "Х" для экспорта.*/
       F7_SUMWITHNDS = nds;
       f12_BAZANDSEXPORT = nds;
       f4_viddoc='МДП';/*Вместо ПН07*/
     end

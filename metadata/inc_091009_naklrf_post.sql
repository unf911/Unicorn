...
CREATE OR ALTER PROCEDURE NAKLRF_POST_PC(
    ID_NAKL INTEGER,
    ID_SCHET INTEGER)
AS
...
begin
  ...
  if (tip<>classid) then begin
    execute procedure error('Нельзя провести не расходную накладную на финуслуги');
  end
  ...
  if (/*$$IBEC$$ (:posted = 1) and $$IBEC$$*/ (id_schet is null)) then begin
    execute procedure naklrf_unpost_pc(:id_nakl);
  end
  ...
  /*проводки*/
  if ((id_schet is null) or (schet_name='ДОЛГИ ЗА ФИНУСЛУГИ') ) then begin
    if (id_xoz_oper<>id_xoz_oper_fact) then begin
    ...
    if (id_xoz_oper=id_xoz_oper_bux) then begin
      ...
      for
        select id_nakl
          ...
          into :id_nakl_depend
      do begin
        execute procedure naklrf_post_pc(:id_nakl_depend,:id_schet_dolg_nal);
...
DESCRIBE PROCEDURE NAKLRF_POST_PC
'Проведение расходной накладной на финуслуги.
Формы ввода-Расходные накладные на услуги.';

GRANT SELECT,UPDATE ON NAKLO TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE ERROR TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE GET_OID_OBJECTS_PC TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLO_CAN_MODIFY_COMMON_PC TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLRF_UNPOST_PC TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE GET_NAME_OBJECTS_PC TO PROCEDURE NAKLRF_POST_PC;

GRANT SELECT,INSERT,DELETE ON PROVODKI TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO PROCEDURE NAKLRF_POST_PC;

GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO PROCEDURE NAKLRF_POST_PC;
GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO SYSDBA;
GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO BOSSES;
GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO BUXG;
GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO MANAGER;
GRANT EXECUTE ON PROCEDURE NAKLRF_POST_PC TO POWER_USER;

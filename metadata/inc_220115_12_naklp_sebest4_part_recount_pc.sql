          ...
          */
          ...
          if (((:tip_depend=103) and (id_sklad_to<>id_sklad_in)) or (tip_depend<>103)) then begin
            ...
            select p.id_partiya, p.sebest
              from naklot_sebest2_post_pc(:id_nakl_depend, :id_nakld_recount, 3) p
              into :id_partiya, :sebest;
          end
          if ((:tip_depend=103) and (id_sklad_to<>id_sklad_in)) then begin

            select p.id_partiya, p.sebest
              from naklot_sebest2_post_pc(:id_nakl_depend, :id_nakld_recount, 3) p
              into :id_partiya, :sebest;
            /*kredit_kolotp_depend>0  т.е. на хранение сейчас выглядит
            как расход, поэтому проваливаемся в рекурсию которая смотрит
            на него как на приход, его не трогает, пересчитывает зависимые*/
            SELECT id_sklad_to
              FROM naklo om
              WHERE om.id_nakl=:id_nakl_depend
              INTO :id_sklad_to;

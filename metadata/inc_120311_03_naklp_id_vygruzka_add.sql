...
ALTER PROCEDURE REP_SEBEST_KREDIT_PO_NAKL_PC(
    DAT_FROM DATE,
    DAT_TO DATE)
RETURNS (
    ID VARCHAR(50),
...
begin
  ...
  insert into naklo (id_nakl,id,kurs,nds,dat,id_izg,id_zak,
      ...
      SCH, DOV, DDO, LICO, FR, ID_VYGRUZKA
    ...
    values (new.id_nakl,new.id,new.kurs,new.nds,new.dat,new.id_izg,new.id_zak,
      ...
      new.sch,new.dov,new.ddo,new.lico,new.fr, new.id_vygruzka);
end^
...
/*$$IBEC$$       if (Round(:nalog_nds_parent-:nalog_nds,6)<>0) then begin
    ...
    select oid
      from GET_OID_OBJECTS_PC('ДОХОД ОТ НЕДОПОСТАВОК',:id_statya_dr_tip)
      into :id_statya_dox;
    select oid
    ...
    then begin
      ...
      execute procedure naklo_auto_spend_pc(:id_schet_post,:id_nakl,:dat, :id_izg,:id_zak, :id_currency_to, 1);
    ...
    then begin
      ...
      values(:id_schet_post_uslug,:dat,:tip,
        :nds, :id_nakl,
        :id_izg,:id_zak,:id_manager,:id_nakl,:id_currency_to);
      execute procedure naklo_auto_spend_pc(:id_schet_post_uslug,:id_nakl,
        :dat, :id_izg,:id_zak, :id_currency_to, :tip_transh);
    end  /*if РАСЧЁТЫ С ПОСТАВЩИКАМИ*/    
    ...
    begin
      ...
      do begin
        ...
        execute procedure naklp_post_pc(:depend_id_nakl,:id_schet_finrez);
      end
    end   /*schet_name='СЕБЕСТОИМОСТЬ СКЛАД'*/
    
    if ((id_schet is null) or (schet_name='ФИНАНСОВЫЙ РЕЗУЛЬТАТ') ) then begin
      if (xoz_oper_name='ПРИХОДНАЯ БУХ НАКЛАДНАЯ НА ТОВАР') then begin
        ...
        values(:id_schet_finrez,:dat,:tip,
          ...
          :id_project,:id_statya_dox,:id_currency,:id_zak,:id_izg,:id_manager);
      end /*if xoz_oper*/
    end /*if id_schet*/
...
GRANT SELECT ON NAKLP_VW TO PROCEDURE REP_SEBEST_KREDIT_PO_NAKL_PC;
...
GRANT EXECUTE ON PROCEDURE NAKLO_AUTO_SPEND_PC TO PROCEDURE NAKLPBUX_POST_PC;
GRANT EXECUTE ON PROCEDURE NAKLP_POST_PC TO PROCEDURE NAKLPBUX_POST_PC;
GRANT EXECUTE ON PROCEDURE GET_SUM_TOVARWITHNDS TO PROCEDURE NAKLPBUX_POST_PC;
...
GRANT SELECT ON NAKLP_VW TO PROCEDURE REP_SEBEST_KREDIT_PO_NAKL_PC;
GRANT SELECT ON NAKLP_DET_VW TO PROCEDURE REP_SEBEST_KREDIT_PO_NAKL_PC;
GRANT SELECT ON SPR_SCHET_VW TO PROCEDURE REP_SEBEST_KREDIT_PO_NAKL_PC;
GRANT EXECUTE ON PROCEDURE REP_SEBEST_KREDIT_PO_NAKL2_PC TO PROCEDURE REP_SEBEST_KREDIT_PO_NAKL_PC;

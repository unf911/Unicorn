...
select
  ...
  (select a.value1
     from attributes a
     where a.oid=om.id_nakl and a.attribute_id=2) as nomer_prav,
  (select a.value1
     from attributes a
     where a.oid=om.id_nakl and a.attribute_id=3) as vid_perevozok
from naklo om , rdb$database
where om.tip= -106  /*СПРАВОЧНИК РЕЙСОВ*/
;
/*-------------------------------------------------------------------------*/
...
begin
  select oid
    from GET_OID_OBJECTS_PC('СПРАВОЧНИК РЕЙСОВ',-100)
    into :classid;   
  insert into naklo (
    id_nakl,
    id_izg,
  ...
  execute procedure attributes_set2_pc(
    ...
    :classid,
    'Номер прав',
    cast(new.NOMER_PRAV as varchar(250)));
  execute procedure attributes_set2_pc(
    new.id_reys,
    :classid,
    'Вид перевозок',
    cast(new.VID_PEREVOZOK as varchar(250)));
end^
...
GRANT SELECT, UPDATE, DELETE, INSERT, REFERENCES ON SPR_REYS_VW TO MANAGER;

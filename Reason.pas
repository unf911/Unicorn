...
procedure TfrmReason.btnDeleteClick(Sender: TObject);
begin
try
...
procedure TfrmReason.btnInsertClick(Sender: TObject);
begin
try
  cdsSklad.Insert;
  SkladEdit;
...
procedure TfrmReason.SkladEdit;
begin
try
...
begin
  if dmdEx.cdsReason.Active then begin
    dmdEx.cdsReason.Refresh;
  end;
  Action := caFree;
end;
...
begin
  cdsSklad.Open;
  frmSkladEdit := TfrmReasonEdit.Create (self);
  frmSkladEdit.dsSkladEdit.Dataset := cdsSklad;
end;

procedure TfrmReason.UnfDBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
    vk_return :begin
...
procedure TfrmReason.btnEditClick(Sender: TObject);
begin
try
  cdsSklad.Edit;
  SkladEdit;
...
procedure TfrmReason.cdsSkladReconcileError(
...
procedure TfrmReason.actRefreshExecute(Sender: TObject);
begin
try
  cdsSklad.Refresh;
except
...
procedure TfrmReason.cdsSkladAfterInsert(DataSet: TDataSet);
begin
try
  ...
  cdsSklad.FieldByName('classid').asInteger :=
    dmdEx.GetOidObjects('СПРАВОЧНИК ПРИЧИН КОНТАКТОВ',-100)
except
  AssertInternal('294D416F-A6E1-4B2E-8A52-59D36EEFAEF6');
end;
end;

procedure TfrmReason.cdsSkladBeforePost(DataSet: TDataSet);
begin
  if cdsSklad.FieldByName('NAME').IsNull then begin
    showmessage('Введите название причины контакта');
    Abort;
  end;
end;

procedure TfrmReason.dbgClientDblClick(Sender: TObject);
begin
  SkladEdit;
end;

procedure TfrmReason.dbgClientKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
    vk_return :begin
...
function TfrmReason.ShowModalForInsert: integer;
begin
